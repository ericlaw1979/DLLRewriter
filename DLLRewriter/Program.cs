...
namespace DLLRewriter
{
    class Program
    {
        ...
        public static string[] GetChromeFolders()
        {
        ...
        static int Main(string[] args)
        {
            try
            {
                if ((args.Length == 1) && (args[0].ToLower() == "auto"))
                {
                    try
                    {
                        string[] sCandidates = GetChromeFolders();
                        foreach (string sCandidate in sCandidates)
                        {
                            try
                            {
                                RewriteDLL(sCandidate + Path.DirectorySeparatorChar + "chrome.dll");
                            }
                            catch { }
                        }
                    }
                    ...
                    if (args.Length < 1)
                    {
                        ...
                        try
                        {
                            ...
                            foreach (string sCandidate in sCandidates)
                            {
                                ...
                                {
                                    sDefaultPath = sCandidate;
                                }
                                ofd.CustomPlaces.Add(sCandidate);
                            }
                        }
                        catch { /*non-critical*/ }
                        ...
                        {
                            Console.WriteLine("Usage:\n\n\tDLLRewriter.exe [auto|<path-to-chrome.dll>]");
                            return 1;
                        }
                    }
                    else
        ...
        private static void RewriteDLL(string sFilename)
        {
            ...
            for (int iX = 0; iX < arrFile.Length - arrCmdId.Length; iX++)
            {
                if (arrFile[iX] == arrCmdId[0])
                {
                    if (MatchingSequence(arrCmdId, arrFile, iX))
                    {
                        ...
                        Console.WriteLine("0x{0:x} cmdId {1}", iX, "\nChanging 0xC3 0x88 to 0xDF 0x84 to change CAST to CLOSETAB");
